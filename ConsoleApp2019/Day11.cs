...
namespace ConsoleApp2019
{
    class Day11 : IDay
    {
        ...
        private Dictionary<Point<int>, long> surface;
        ...
        private void Paint(int startColor)
        {
            surface = new Dictionary<Point<int>, long>();
            var location = new Point<int>(0, 0);
            surface.Add(location, startColor);
                ...
                while (run)
                {
                    ...
                    switch (orientation)
                    {
                        case 0: // up
                            ...
                            location = new Point<int>(location.x + (direction == 0 ? -1 : 1), location.y);
                            break;
                        case 1: // right
                            orientation = direction == 0 ? 0 : 2;
                            location = new Point<int>(location.x, location.y + (direction == 0 ? -1 : 1));
                            break;
                        case 2: // down
        ...
        {
            var minx = surface.Keys.Min(p => p.x);
            var maxx = surface.Keys.Max(p => p.x);

            var miny = surface.Keys.Min(p => p.y);
            var maxy = surface.Keys.Max(p => p.y);
            ...
            foreach (var y in yrange)
            {
                ...
                foreach (var x in xrange)
                {
                   // Console.WriteLine($"{x},{y}");
                    ...
                    if (surface.ContainsKey(point))
                    {
                        var color = surface[point] == 0 ? ' ' : 'o';
                        Console.Write(color);
                    }
                    else
                    {
