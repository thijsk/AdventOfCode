...
namespace ConsoleApp2015
{
    ...
    {
        private string input = @"toggle 461,550 through 564,900
...
turn on 222,12 through 856,241";
        ...
        public long Part1()
        {
            ...
            {
                var groups = instruction.Split(' ');
                if (instruction.StartsWith("turn on"))
                {
                    var pos1 = groups[2].Split(',');
                    var pos2 = groups[4].Split(',');

                    for (int x = int.Parse(pos1[0]); x <= int.Parse(pos2[0]); x++)
                    for (int y = int.Parse(pos1[1]); y <= int.Parse(pos2[1]); y++)
                    {
                        lights[x, y] = true;
                    }
                ...
                {
                    var pos1 = groups[1].Split(',');
                    var pos2 = groups[3].Split(',');
                    for (int x = int.Parse(pos1[0]); x <= int.Parse(pos2[0]); x++)
                    for (int y = int.Parse(pos1[1]); y <= int.Parse(pos2[1]); y++)
                    {
