...
namespace ConsoleApp2015
{
    ...
    class Day16 : IDay
    {
        private string input = @"Sue 1: children: 1, cars: 8, vizslas: 7
...
Sue 500: cars: 1, perfumes: 6, vizslas: 1";
        ...
        private string input2 = @"children: 3
cats: 7
samoyeds: 2
pomeranians: 3
akitas: 0
...
perfumes: 1";
        ...
        public long Part1()
        {
            ...
            {
                var match = true;
                foreach (var attribute in aunt.Values)
                {
                    if (ticker[attribute.Key] != attribute.Value)
                        match = false;
                }
                if (match)
                {
        ...
        private Dictionary<string, int> ParseInput2()
        {
            var result = new Dictionary<string, int>();

            foreach (var attribute in input2.Split('\n'))
            {
                var kv = attribute.Trim().Split(':');
                result.Add(kv[0].Trim(), int.Parse(kv[1].Trim()));
            }

            return result;
        }

        private IEnumerable<AuntSue> ParseInput()
        {
            List<AuntSue> result = new List<AuntSue>();
            foreach (var txt in input.Split('\n'))
            {
                var aunt = new AuntSue();
                aunt.Number = int.Parse(txt.Split(':')[0].Split(' ')[1]);
                var allattributestxt = txt.Substring(txt.IndexOf(':') + 1).Trim();
                foreach (var attributestxt in allattributestxt.Split(','))
                {
                    var attributetxt = attributestxt.Split(':');
                    var name = attributetxt[0].Trim();
                    var value = int.Parse(attributetxt[1].Trim());
                    aunt.Values.Add(name, value);
                }
                result.Add(aunt);
            }
        ...
        public long Part2()
        {
            ...
            foreach (var aunt in aunts)
            {
                ...
                foreach (var attribute in aunt.Values)
                {
                    if (attribute.Key == "cats" || attribute.Key == "trees")
                    {
                        if (ticker[attribute.Key] >= attribute.Value)
                            match = false;
                    } else if (attribute.Key == "pomeranians" || attribute.Key == "goldfish")
                    {
                        if (ticker[attribute.Key] <= attribute.Value)
                            match = false;
                    } else if (ticker[attribute.Key] != attribute.Value)
                        match = false;
                }
                if (match)
                {
