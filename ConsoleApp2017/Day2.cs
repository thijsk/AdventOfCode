...
namespace ConsoleApp2017
{
    class Day2 : IDay
    {
        private string input = @"3093	749	3469	142	2049	3537	1596	3035	2424	3982	3290	125	249	131	118	3138
...
127	375	330	257	220	295	145	335	304	165	151	141	289	256	195	272";
        ...
        public long Part1()
        {
            ...
            foreach (var line in linetext)
            {
                ...
                var columnMax = int.MinValue;
                foreach (var column in columntext)
                {
                    var value = int.Parse(column.Trim());
                    columnMin = Math.Min(columnMin, value);
                    columnMax = Math.Max(columnMax, value);
                }
                var columnSum = columnMax - columnMin;
                sum += columnSum;
            }
        ...
        public long Part2()
        {
            ...
            foreach (var line in linetext)
            {
                ...
                {
                    for (int j = 0; j < columntext.Length; j++)
                    {
                        if (i == j)
                            continue;

                        var first = int.Parse(columntext[i].Trim());
                        var second = int.Parse(columntext[j].Trim());
                        double resultD = (double)first / (double)second;
